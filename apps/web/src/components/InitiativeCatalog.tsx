...
function InitiativeCard({ initiative, catalog, decisions, preview, busy, add, remove, compared, toggleCompare }: { initiative: Initiative; catalog: Catalog; decisions: Decision[]; preview: Preview; busy: boolean; add: (decision: Decision) => void; remove: (id: string) => void; compared: boolean; toggleCompare: () => void }) {
  ...
  const whatIf = !selected && scopeKey ? preview.what_if?.[initiative.id]?.[scopeKey] : null;
  return <article className={`initiative-card domain-${initiative.domain} ${selected ? 'is-selected' : ''}`} data-testid={`initiative-${initiative.id}`}>
    <div className="initiative-top"><span className="domain-icon"><Icon name={domain.icon}/></span><span className="initiative-id">{initiative.id}</span><button type="button" className={`compare-button ${compared ? 'is-active' : ''}`} aria-pressed={compared} onClick={toggleCompare}>{compared ? 'Сравнивается' : 'Сравнить'}</button>{selected ? <span className="selected-badge"><Icon name="check" size={13}/>В сценарии</span> : <span className="scope-badge">{initiative.scope === 'citywide' ? 'Весь город' : 'Один район'}</span>}</div>
    ...
    <div className="initiative-bottom">{initiative.scope === 'district' ? <DistrictSelect label={`Район для ${initiative.id}`} districts={catalog.districts} value={selected?.district_id ?? district} onChange={setDistrict} disabled={!!selected || busy} options={selected ? undefined : options}/> : <div className="citywide-label"><Icon name="city" size={17}/><span>Эффект во всех 5 районах</span></div>}
      ...
      <p id={`reason-${initiative.id}`} className={`availability-reason ${reason && scopeKey ? 'has-reason' : ''}`}>{selected ? 'Район можно изменить в вашем сценарии' : busy ? 'Проверяем доступность…' : reason || (whatIf ? `Что если: Score ${formatNumber(whatIf.projected_score, 2)} · ${signed(whatIf.score_change)}` : `Реализуется ${formatNumber(realized * 100)}% полного эффекта`)}</p>
    </div>
  </article>;
}
...
export function InitiativeCatalog({ catalog, decisions, preview, filter, setFilter, busy, add, remove }: { catalog: Catalog; decisions: Decision[]; preview: Preview; filter: Domain | 'all'; setFilter: (filter: Domain | 'all') => void; busy: boolean; add: (decision: Decision) => void; remove: (id: string) => void }) {
  ...
  return <section id="initiatives" className="catalog-section" aria-labelledby="catalog-title">
    ...
    {compared.length > 0 && <div className="comparison-tray"><div><span className="eyebrow">СРАВНЕНИЕ ИНИЦИАТИВ</span><strong>{compared.length === 1 ? 'Выберите ещё одну карточку' : 'Цена, скорость и эффекты рядом'}</strong></div>{compared.map((id) => { const item = catalog.initiatives.find((initiative) => initiative.id === id)!; return <article key={id} className={`domain-${item.domain}`}><span className="domain-icon"><Icon name={domains.find((domain) => domain.id === item.domain)!.icon} size={16}/></span><div><strong>{item.name_ru}</strong><small>{item.cost} ед. · лаг {item.lag_quarters} кв. · {Object.entries(item.effects).map(([code, value]) => `${indicatorLabels[code as IndicatorCode]} ${signed(value)}`).join(' · ')}</small></div><button type="button" className="icon-button" aria-label={`Убрать ${id} из сравнения`} onClick={() => setCompared((value) => value.filter((itemId) => itemId !== id))}><Icon name="close" size={15}/></button></article>;})}</div>}
    {initiatives.length ? <div className="initiative-grid">{initiatives.map((initiative) => <InitiativeCard key={initiative.id} {...{ initiative, catalog, decisions, preview, busy, add, remove }} compared={compared.includes(initiative.id)} toggleCompare={() => setCompared((value) => value.includes(initiative.id) ? value.filter((id) => id !== initiative.id) : [...value.slice(-1), initiative.id])}/>)}</div> : <div className="panel"><EmptyState icon="search" title="Инициативы не найдены"><p>Измените запрос или сбросьте фильтры.</p><Button onClick={() => { setQuery(''); setFilter('all'); setSelectedOnly(false); searchRef.current?.focus(); }}>Сбросить фильтры</Button></EmptyState></div>}
