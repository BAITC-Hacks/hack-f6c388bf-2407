...
export function CityMap({ districts, active, onSelect }: { districts: District[]; active: DistrictId; onSelect: (id: DistrictId) => void }) {
  ...
  const visible = districts.map((district) => view === 'before' ? { ...district, score: district.baseline_score, delta: 0, indicators: district.before } : district);
  const selected = visible.find((district) => district.id === active)!;
  return <section className="panel city-panel" aria-labelledby="map-title">
    <div className="panel-heading"><div><div className="eyebrow">ЦИФРОВОЙ ДВОЙНИК</div><h2 id="map-title">Астана сейчас и через 2 года</h2></div><div className="map-view-toggle" role="group" aria-label="Состояние карты"><button type="button" aria-pressed={view === 'before'} onClick={() => setView('before')}>Сейчас</button><button type="button" aria-pressed={view === 'after'} onClick={() => setView('after')}>После решений</button></div></div>
    <div className="city-map">
      <svg viewBox="0 0 700 410" role="img" aria-label="Условная схема пяти районов Астаны. Оценки и выбор района доступны кнопками под схемой.">
        ...
        {shapes.map((shape) => {
          const district = visible.find((item) => item.id === shape.id)!;
          return <g key={shape.id} className={`map-district ${shape.id === active ? 'is-active' : ''} ${district.score < 50 ? 'is-vulnerable' : ''} ${view === 'after' && district.delta > 0 ? 'is-improved' : ''}`}>
    ...
    <div className="district-chips" role="group" aria-label="Выберите район для просмотра">{visible.map((district) => <button type="button" key={district.id} onClick={() => onSelect(district.id)} aria-pressed={active === district.id}>{district.name_ru}{view === 'after' && district.delta !== 0 && <span>{signed(district.delta, 1)}</span>}</button>)}</div>
