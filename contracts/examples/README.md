...
Вложенные districts/initiative_contributions ещё не детализированы в HTTP-контракте.
Здесь приведён конкретный формат для AI-модуля; backend должен его поддержать или
сделать адаптер. В realized_effects синергии не дублируются. Вклад в итоговый score
